...
</x-app><x-app title="Progres Latihan">
  <!-- Main Content -->
  <main class="flex-1 p-6 overflow-y-auto">
    ...
    <form method="GET" class="mb-6">
      <label class="block font-semibold text-gray-600 mb-1">Pilih Trainee</label>
      <select name="trainee_id" onchange="this.form.submit()" class="border rounded p-2 w-full max-w-sm">
        <option value="">-- Pilih Trainee --</option>
        @foreach($trainees as $trainee)
          <option value="{{ $trainee->id }}" {{ $traineeId == $trainee->id ? 'selected' : '' }}>
            {{ $trainee->name }}
    ...
    <script>
      ...
      new Chart(document.getElementById('latihanChart'), {
        type: 'bar',
        data: {
          labels: tanggalLatihan,
          datasets: [{
      ...
      new Chart(document.getElementById('makanChart'), {
        type: 'bar',
        data: {
          labels: tanggalMakan,
          datasets: [{
