...
<x-app title="Beranda">
  ...
  {{-- Tambahan Tabel Daftar Trainee --}}
  <div class="mt-10">
    <h2 class="text-xl font-semibold mb-4 text-left">Daftar Trainee</h2>
    <div class="overflow-x-auto">
      <table class="min-w-full bg-white border border-gray-200 rounded-xl overflow-hidden text-left">
        <thead class="bg-green-200 text-gray-700">
          <tr>
            <th class="py-2 px-4 border-b">Nama</th>
            <th class="py-2 px-4 border-b">Email</th>
            <th class="py-2 px-4 border-b text-center">Total Latihan</th>
          </tr>
        </thead>
        <tbody>
          @forelse ($trainees as $trainee)
            <tr class="hover:bg-gray-100">
              <td class="py-2 px-4 border-b">{{ $trainee->name }}</td>
              <td class="py-2 px-4 border-b">{{ $trainee->email }}</td>
              <td class="py-2 px-4 border-b text-center">{{ $trainee->total_latihan }}</td>
            </tr>
          @empty
            <tr>
